...
#!/bin/bash
# FirewallFalcon limiter version 2026-04-03.1
...
mkdir -p "$BW_DIR" "$PID_DIR" "$BAN_DIR"
shopt -s nullglob

# ---------------------------------------------------------------------------
# get_net_bytes_for_pid PID
#   Returns total network rx+tx bytes for a given PID by summing all
#   /proc/<pid>/net/dev interface counters (excludes lo).
#   Falls back to 0 if the file is unreadable.
# ---------------------------------------------------------------------------
get_net_bytes_for_pid() {
    local pid="$1"
    local net_dev="/proc/$pid/net/dev"
    local total=0
    [[ -r "$net_dev" ]] || { echo 0; return; }
    while IFS= read -r line; do
        # Lines look like: "  eth0:  1234 ... 5678 ..."
        # Field 1 = iface:, field 2 = rx_bytes, field 10 = tx_bytes
        [[ "$line" =~ ^[[:space:]]*([^:]+):[[:space:]]+([0-9]+)[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+([0-9]+) ]] || continue
...
while true; do
    ...
    while IFS=: read -r user pass expiry limit bandwidth_gb _extra; do
        ...
        for pid in "${!unique_pids[@]}"; do
            # FIX: Use per-PID network namespace bytes (rx+tx) instead of
            # /proc/PID/io which measures disk I/O, not network traffic.
            cur=$(get_net_bytes_for_pid "$pid")
            [[ "$cur" =~ ^[0-9]+$ ]] || cur=0
            ...
            if [[ -f "$pidfile" ]]; then
                ...
                if (( cur >= prev )); then
                    ...
                    # Counter wrapped or PID was reused — treat as fresh start
                    d=0
                fi
        ...
        # Clean up stale pidfiles for PIDs that no longer exist
...
sync_runtime_components_if_needed() {
    local limiter_marker="# FirewallFalcon limiter version 2026-04-03.1"
