...
delete_firewallfalcon_user_accounts() {
    ...
    for username in "${users_to_delete[@]}"; do
        ...
        rm -f "$BANDWIDTH_DIR/${username}.ipt_last"
        rm -rf "$BANDWIDTH_DIR/pidtrack/${username}"
        # Usuń reguły iptables zliczania pasma dla tego użytkownika
        local _uid
        _uid=$(id -u "$username" 2>/dev/null) || true
        if [[ -n "$_uid" ]]; then
            iptables -D OUTPUT -m owner --uid-owner "$_uid" \
                -m comment --comment "ff_bw_${username}" -j ACCEPT 2>/dev/null || true
            iptables -D INPUT -m owner --uid-owner "$_uid" \
                -m comment --comment "ff_bw_${username}" -j ACCEPT 2>/dev/null || true
        fi
    done
    ...
    if [[ -f "$DB_FILE" ]]; then
        local db_tmp
        db_tmp=$(mktemp)
        if awk -F: 'NR==FNR { drop[$1]=1; next } !($1 in drop)' <(printf "%s\n" "${users_to_delete[@]}") "$DB_FILE" > "$db_tmp"; then
            mv "$db_tmp" "$DB_FILE"
        else
            rm -f "$db_tmp"
        fi
...
BAN_DIR="/etc/firewallfalcon/bans"
IPT_COMMENT_PREFIX="ff_bw_"
SCAN_INTERVAL=30
BAN_DURATION=60

mkdir -p "$BW_DIR" "$BAN_DIR"
shopt -s nullglob

# ---------------------------------------------------------------------------
# SYSTEM ZLICZANIA PASMA — iptables per-UID (owner match)
#
# Każdy użytkownik z limitem GB ma dwie reguły iptables:
#   OUTPUT  — ruch wychodzący (dane wysyłane do klienta SSH)
#   INPUT   — ruch przychodzący (dane odbierane od klienta SSH)
# Reguły są dodawane przy pierwszym napotkaniu użytkownika w pętli.
# Bajty są odczytywane przez "iptables -nxvL" i akumulowane w pliku .usage.
# ---------------------------------------------------------------------------

# Sprawdź czy reguła iptables dla użytkownika już istnieje
ipt_rule_exists() {
    local user="$1"
    local uid
    uid=$(id -u "$user" 2>/dev/null) || return 1
    iptables -nxvL OUTPUT 2>/dev/null | grep -q "ff_bw_${user}"
}

# Dodaj reguły OUTPUT + INPUT dla użytkownika (jeśli nie istnieją)
ipt_add_user_rules() {
    local user="$1"
    local uid
    uid=$(id -u "$user" 2>/dev/null) || return 1

    if ! iptables -nxvL OUTPUT 2>/dev/null | grep -q "ff_bw_${user}"; then
        iptables -I OUTPUT -m owner --uid-owner "$uid" \
            -m comment --comment "${IPT_COMMENT_PREFIX}${user}" -j ACCEPT 2>/dev/null || true
    fi
    if ! iptables -nxvL INPUT 2>/dev/null | grep -q "ff_bw_${user}"; then
        iptables -I INPUT -m owner --uid-owner "$uid" \
            -m comment --comment "${IPT_COMMENT_PREFIX}${user}" -j ACCEPT 2>/dev/null || true
    fi
}

# Usuń reguły iptables dla użytkownika (przy usuwaniu konta)
ipt_remove_user_rules() {
    local user="$1"
    local uid
    uid=$(id -u "$user" 2>/dev/null) || return 0

    # Usuń wszystkie pasujące reguły (może być więcej niż jedna przy duplikacji)
    while iptables -nxvL OUTPUT 2>/dev/null | grep -q "ff_bw_${user}"; do
        iptables -D OUTPUT -m owner --uid-owner "$uid" \
            -m comment --comment "${IPT_COMMENT_PREFIX}${user}" -j ACCEPT 2>/dev/null || break
    done
    while iptables -nxvL INPUT 2>/dev/null | grep -q "ff_bw_${user}"; do
        iptables -D INPUT -m owner --uid-owner "$uid" \
            -m comment --comment "${IPT_COMMENT_PREFIX}${user}" -j ACCEPT 2>/dev/null || break
    done
}

# Odczytaj bajty z reguł iptables dla użytkownika (OUTPUT + INPUT)
# Zwraca sumę bajtów ze wszystkich pasujących reguł
ipt_get_bytes() {
    local user="$1"
    local total=0
    local bytes
    ...
    done < <(iptables -nxvL OUTPUT 2>/dev/null | grep "ff_bw_${user}" || true)

    # INPUT
    while IFS= read -r line; do
        bytes=$(echo "$line" | awk '{print $2}')
        [[ "$bytes" =~ ^[0-9]+$ ]] && total=$(( total + bytes ))
    done < <(iptables -nxvL INPUT 2>/dev/null | grep "ff_bw_${user}" || true)

    echo "$total"
}

# Zeruj liczniki iptables dla użytkownika (bez usuwania reguł)
ipt_reset_counters() {
    local user="$1"
    local uid
    uid=$(id -u "$user" 2>/dev/null) || return 0
...
# Przy starcie skryptu: upewnij się że moduł xt_owner jest załadowany
modprobe xt_owner 2>/dev/null || true

# Plik przechowujący ostatni odczyt bajtów z iptables per-user
# Format: liczba bajtów zanotowana przy ostatnim skanie
# Akumulacja: usage = poprzedni_usage + (biezace_ipt_bytes - ostatni_odczyt_ipt)
# Dzięki temu zerowanie liczników iptables przy reboocie nie cofa historii
...
while true; do
    ...
    while IFS=: read -r user pass expiry limit bandwidth_gb _extra; do
        ...
        # Check if a timed ban (from session limit overflow) has expired
        ban_file="$BAN_DIR/${user}.ban"
        if [[ -f "$ban_file" ]]; then
            ban_ts=0
            read -r ban_ts < "$ban_file" || ban_ts=0
            if [[ "$ban_ts" =~ ^[0-9]+$ ]] && (( current_ts >= ban_ts )); then
                rm -f "$ban_file"
                if $user_locked; then
                    usermod -U "$user" &>/dev/null
                    user_locked=false
                    unset "locked_users[$user]"
                fi
            fi
        fi
        ...
        if (( online_count > limit )); then
            if ! $user_locked; then
                usermod -L "$user" &>/dev/null
                killall -u "$user" -9 &>/dev/null
                printf "%s\n" "$(( current_ts + BAN_DURATION ))" > "$ban_file"
                locked_users["$user"]=1
                user_locked=true
            else
                killall -u "$user" -9 &>/dev/null
        ...
        # -------------------------------------------------------------------
        # ZLICZANIE PASMA — iptables per-UID
        # Działa zarówno gdy user jest online jak i offline (iptables liczy
        # cały ruch UID na poziomie jądra, niezależnie od aktywnych sesji).
        # -------------------------------------------------------------------
        if [[ -n "$bandwidth_gb" && "$bandwidth_gb" != "0" ]]; then

            # Upewnij się że reguły iptables istnieją dla tego użytkownika
            ipt_add_user_rules "$user"

            usagefile="$BW_DIR/${user}.usage"
            ipt_lastfile="$BW_DIR/${user}.ipt_last"

            # Wczytaj skumulowane zużycie z poprzednich sesji
            accumulated=0
            if [[ -f "$usagefile" ]]; then
            ...
            # Odczytaj bieżące bajty z iptables
            ipt_current=$(ipt_get_bytes "$user")
            [[ "$ipt_current" =~ ^[0-9]+$ ]] || ipt_current=0

            # Odczytaj ostatni zanotowany odczyt iptables
            ipt_last=0
            if [[ -f "$ipt_lastfile" ]]; then
                read -r ipt_last < "$ipt_lastfile"
                [[ "$ipt_last" =~ ^[0-9]+$ ]] || ipt_last=0
            fi

            # Oblicz deltę (różnicę od ostatniego skanu)
            # Jeśli ipt_current < ipt_last — liczniki zostały zresetowane
            # (np. reboot, iptables -Z) — traktuj bieżący odczyt jako deltę = 0
            if (( ipt_current >= ipt_last )); then
                delta=$(( ipt_current - ipt_last ))
            else
                delta=0
                # Zapisz nowy punkt bazowy po resecie liczników
            fi

            new_total=$(( accumulated + delta ))

            # Zapisz zaktualizowane wartości
            printf "%s\n" "$new_total" > "$usagefile"
            printf "%s\n" "$ipt_current" > "$ipt_lastfile"

            # Sprawdź czy quota przekroczona
            ...
            # Aktualizuj zmienne dla bannera
            bw_accumulated_for_banner="$new_total"
        else
            bw_accumulated_for_banner=0
        fi
        ...
        if $dynamic_banners_enabled; then
            ...
            if [[ "$bandwidth_gb" != "0" && -n "$bandwidth_gb" ]]; then
                used_gb=$(awk "BEGIN {printf \"%.2f\", $bw_accumulated_for_banner / 1073741824}")
            ...
            banner_content+="<font color=\"white\">📅 <b>Data waznosci :</b> $expiry ($days_left)</font><br>"
            banner_content+="<font color=\"white\">📊 <b>Ilosc GB  :</b> $bw_info</font><br>"
            banner_content+="<font color=\"white\">🔌 <b>Aktywne urzadzenia   :</b> $online_count/$limit</font><br><br>"
...
db_tmp=$(mktemp)
if awk -F: -v u="$username" '$1 != u' "$DB_FILE" > "$db_tmp"; then
    mv "$db_tmp" "$DB_FILE"
else
    rm -f "$db_tmp"
fi
...
rm -f "$BW_DIR/${username}.ipt_last"
rm -rf "$BW_DIR/pidtrack/${username}"
# Usuń reguły iptables zliczania pasma
_uid=$(id -u "$username" 2>/dev/null) || true
if [[ -n "$_uid" ]]; then
    iptables -D OUTPUT -m owner --uid-owner "$_uid" \
        -m comment --comment "ff_bw_${username}" -j ACCEPT 2>/dev/null || true
...
get_user_status() {
    ...
    local expiry_date; expiry_date=$(grep "^$username:" "$DB_FILE" | cut -d: -f3)
    if passwd -S "$username" 2>/dev/null | grep -q " L "; then echo -e "${C_YELLOW}🔒 Locked${C_RESET}"; return; fi
    if [[ -n "$expiry_date" && "$expiry_date" != "Never" ]]; then
        local expiry_ts; expiry_ts=$(date -d "$expiry_date" +%s 2>/dev/null || echo 0)
        local current_ts; current_ts=$(date +%s)
        if [[ "$expiry_ts" =~ ^[0-9]+$ ]] && (( expiry_ts > 0 && expiry_ts < current_ts )); then
            echo -e "${C_RED}🗓️ Expired${C_RESET}"; return
        fi
    fi
    ...
    while true; do
        ...
        case $edit_choice in
            1)
               ...
               local tmp_db; tmp_db=$(mktemp)
               awk -F: -v u="$username" -v p="$new_pass" -v e="$cur_expiry" -v l="$cur_limit" -v b="$cur_bw" \
                   'BEGIN{OFS=":"} $1==u{$2=p;$3=e;$4=l;$5=b} {print}' "$DB_FILE" > "$tmp_db" && mv "$tmp_db" "$DB_FILE"
            ...
            2) read -p "Enter new duration (in days from today): " days
               if [[ "$days" =~ ^[0-9]+$ ]]; then
                   ...
                   local tmp_db; tmp_db=$(mktemp)
                   awk -F: -v u="$username" -v p="$cur_pass" -v e="$new_expire_date" -v l="$cur_limit" -v b="$cur_bw" \
                       'BEGIN{OFS=":"} $1==u{$2=p;$3=e;$4=l;$5=b} {print}' "$DB_FILE" > "$tmp_db" && mv "$tmp_db" "$DB_FILE"
            ...
            3) read -p "Enter new simultaneous connection limit: " new_limit
               if [[ "$new_limit" =~ ^[0-9]+$ ]]; then
                   local tmp_db; tmp_db=$(mktemp)
                   awk -F: -v u="$username" -v p="$cur_pass" -v e="$cur_expiry" -v l="$new_limit" -v b="$cur_bw" \
                       'BEGIN{OFS=":"} $1==u{$2=p;$3=e;$4=l;$5=b} {print}' "$DB_FILE" > "$tmp_db" && mv "$tmp_db" "$DB_FILE"
            ...
            4) read -p "Enter new bandwidth limit in GB (0 = unlimited): " new_bw
               if [[ "$new_bw" =~ ^[0-9]+\.?[0-9]*$ ]]; then
                   local tmp_db; tmp_db=$(mktemp)
                   awk -F: -v u="$username" -v p="$cur_pass" -v e="$cur_expiry" -v l="$cur_limit" -v b="$new_bw" \
                       'BEGIN{OFS=":"} $1==u{$2=p;$3=e;$4=l;$5=b} {print}' "$DB_FILE" > "$tmp_db" && mv "$tmp_db" "$DB_FILE"
                   local bw_msg="Unlimited"; [[ "$new_bw" != "0" ]] && bw_msg="${new_bw} GB"
                   echo -e "\n${C_GREEN}✅ Bandwidth limit for '$username' set to ${C_YELLOW}$bw_msg${C_RESET}."
                   # Unlock user if they were locked due to bandwidth and new quota is sufficient
                   if [[ "$new_bw" == "0" ]]; then
                       usermod -U "$username" &>/dev/null
                   elif [[ -f "$BANDWIDTH_DIR/${username}.usage" ]]; then
                       ...
                       if (( used_bytes < new_quota_bytes )); then
                           usermod -U "$username" &>/dev/null
                       fi
                   fi
...
list_users() {
    ...
    # Build array of rows with session count prefix for sorting (DESC by sessions)
    local -a rows=()
    while IFS=: read -r user pass expiry limit bandwidth_gb _extra; do
        [[ -z "$user" || "$user" == \#* ]] && continue
        local online_count="${SSH_SESSION_COUNTS[$user]:-0}"
        local connection_string="$online_count / $limit"
        # FIX: plain_status starts as "Active" (was "Act" before — wrong value used in comparisons)
        ...
        # FIX: was checking "Active" but plain_status was "Act" — now both match
        ...
        # Store row as: zero-padded session count (desc sort key) + TAB + display fields
        # We'll sort numerically descending by session count
        rows+=("$(printf '%010d\t%s\t%s\t%s\t%s\t%s\t%s\t%s' \
            "$online_count" \
            "$user" "$expiry" "$connection_string" "$bw_string" \
            "$plain_status" "$line_color" "$(printf '%s' "$status")")")
    done < "$DB_FILE"

    # Sort by session count descending (reverse numeric on field 1)
    while IFS=$'\t' read -r _cnt user expiry connection_string bw_string plain_status line_color status; do
...
renew_user() {
    ...
    for u in "${SELECTED_USERS[@]}"; do
        ...
        local tmp_db; tmp_db=$(mktemp)
        awk -F: -v u="$u" -v p="$pass" -v e="$new_expire_date" -v l="$limit" -v b="$bw" \
            'BEGIN{OFS=":"} $1==u{$2=p;$3=e;$4=l;$5=b} {print}' "$DB_FILE" > "$tmp_db" && mv "$tmp_db" "$DB_FILE"
...
cleanup_expired() {
    ...
    if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
        ...
        # Remove all expired users from DB in one atomic awk pass
        local db_tmp; db_tmp=$(mktemp)
        if awk -F: 'NR==FNR { drop[$1]=1; next } !($1 in drop)' \
            <(printf "%s\n" "${expired_users[@]}") "$DB_FILE" > "$db_tmp"; then
            mv "$db_tmp" "$DB_FILE"
        else
            rm -f "$db_tmp"
        fi
...
view_user_bandwidth() {
    ...
    if [[ "$bandwidth_gb" == "0" ]]; then
        ...
        # Progress bar
        local bar_width=30
        local filled; filled=$(awk "BEGIN {v=int(($percentage / 100) * $bar_width); if(v>$bar_width)v=$bar_width; print v}")
        local empty=$((bar_width - filled))
        local bar_color="$C_GREEN"
        local pct_int="${percentage%.*}"
        [[ -z "$pct_int" || ! "$pct_int" =~ ^[0-9]+$ ]] && pct_int=0
        if (( pct_int > 80 )); then bar_color="$C_RED"
        elif (( pct_int > 50 )); then bar_color="$C_YELLOW"
        fi
...
user_accounts_menu() {
    while true; do
        clear; show_banner
        echo -e "   ${C_TITLE}═════════════════[ ${C_BOLD}👤 KONTA UŻYTKOWNIKÓW ${C_RESET}${C_TITLE}]═════════════════${C_RESET}"

        if [[ ! -s "$DB_FILE" ]]; then
            echo -e "\n${C_YELLOW}ℹ️ Brak użytkowników w bazie.${C_RESET}"
            echo -e "\n  ${C_WARN}[ 0]${C_RESET} ↩️ Powrót"
            read -r -p "$(echo -e "${C_PROMPT}👉 Wybierz opcję: ${C_RESET}")" _
            return
        fi
        ...
        printf "\n  ${C_CHOICE}[ 1]${C_RESET} Wyświetl wszystkich użytkowników\n"
        printf "  ${C_CHOICE}[ 2]${C_RESET} Szukaj użytkownika po nazwie\n"
        echo -e "  ${C_WARN}[ 0]${C_RESET} ↩️ Powrót do menu głównego\n"

        local sub_choice
        if ! read -r -p "$(echo -e "${C_PROMPT}👉 Wybierz opcję: ${C_RESET}")" sub_choice; then
            echo; return
        fi
        ...
        case "$sub_choice" in
            0) return ;;
            2)
                read -r -p "$(echo -e "${C_CYAN}🔍 Wpisz nazwę użytkownika (lub fragment): ${C_RESET}")" search_term
                if [[ -z "$search_term" ]]; then
                    echo -e "${C_RED}❌ Nie podano frazy.${C_RESET}"; sleep 1; continue
                fi
                ;;
            1|"") search_term="" ;;
            *)
                echo -e "${C_RED}❌ Nieprawidłowa opcja.${C_RESET}"; sleep 1; continue ;;
        esac

        clear; show_banner

        local header_title="Wszyscy użytkownicy"
        [[ -n "$search_term" ]] && header_title="Wyniki dla: \"$search_term\""
        echo -e "   ${C_TITLE}══[ ${C_BOLD}👤 $header_title ${C_RESET}${C_TITLE}]══${C_RESET}\n"

        local sep="${C_CYAN}$(printf '─%.0s' {1..85})${C_RESET}"
        echo -e "$sep"
        printf "${C_BOLD}${C_WHITE}  %-18s %-20s %-16s %-12s %-12s${C_RESET}\n" \
            "UŻYTKOWNIK" "HASŁO" "WYGASA (dd-mm-rrrr)" "DNI DO KOŃCA" "STATUS"
        echo -e "$sep"

        local found=0
        while IFS=: read -r user pass expiry limit bw _extra; do
            [[ -z "$user" || "$user" == \#* ]] && continue
            if [[ -n "$search_term" ]] && ! [[ "${user,,}" == *"${search_term,,}"* ]]; then
                continue
            fi
            found=1

            # Format expiry date to dd-mm-yyyy
            local expiry_display="Nigdy"
            local days_left_str="${C_GRAY}∞${C_RESET}"
            if [[ -n "$expiry" && "$expiry" != "Never" ]]; then
                local exp_ts; exp_ts=$(date -d "$expiry" +%s 2>/dev/null || echo 0)
                if [[ "$exp_ts" =~ ^[0-9]+$ && "$exp_ts" -gt 0 ]]; then
                    expiry_display=$(date -d "$expiry" +%d-%m-%Y 2>/dev/null || echo "$expiry")
                    local diff_s=$(( exp_ts - current_ts ))
                    if (( diff_s <= 0 )); then
                        days_left_str="${C_RED}WYGASŁO${C_RESET}"
                    else
                        local d_left=$(( diff_s / 86400 ))
                        if (( d_left <= 3 )); then
                            days_left_str="${C_RED}${d_left}d${C_RESET}"
                        elif (( d_left <= 7 )); then
                            days_left_str="${C_YELLOW}${d_left}d${C_RESET}"
                        else
                            days_left_str="${C_GREEN}${d_left}d${C_RESET}"
                        fi
                    fi
                fi
            fi
            ...
            # Account status
            local status_str="${C_GREEN}Aktywne${C_RESET}"
            if passwd -S "$user" 2>/dev/null | grep -q " L "; then
                status_str="${C_YELLOW}Zablokowane${C_RESET}"
            fi
            if [[ -n "$expiry" && "$expiry" != "Never" ]]; then
                local exp_ts2; exp_ts2=$(date -d "$expiry" +%s 2>/dev/null || echo 0)
                if [[ "$exp_ts2" =~ ^[0-9]+$ ]] && (( exp_ts2 > 0 && exp_ts2 < current_ts )); then
                    status_str="${C_RED}Wygasłe${C_RESET}"
                fi
            fi

            printf "  ${C_WHITE}%-18s${C_RESET} ${C_YELLOW}%-20s${C_RESET} ${C_CYAN}%-16s${C_RESET} %-12b %-12b\n" \
                "$user" "$pass" "$expiry_display" "$days_left_str" "$status_str"
        done < "$DB_FILE"

        echo -e "$sep"

        if [[ "$found" -eq 0 ]]; then
            echo -e "\n  ${C_YELLOW}ℹ️ Nie znaleziono użytkowników pasujących do frazy \"$search_term\".${C_RESET}"
        fi

        echo -e "\n  ${C_WARN}[ 0]${C_RESET} ↩️ Powrót   ${C_CHOICE}[ 2]${C_RESET} Nowe wyszukiwanie\n"
        local nav_choice
        if ! read -r -p "$(echo -e "${C_PROMPT}👉 Wybierz: ${C_RESET}")" nav_choice; then
            echo; return
        fi
        case "$nav_choice" in
            0) return ;;
...
update_script() {
    clear; show_banner
    echo -e "   ${C_TITLE}═════════════════[ ${C_BOLD}🔄 AKTUALIZACJA SKRYPTU ${C_RESET}${C_TITLE}]═════════════════${C_RESET}"
    echo -e "\n${C_CYAN}Pobieranie najnowszej wersji z GitHub...${C_RESET}\n"

    local tmp_script; tmp_script=$(mktemp)
    if wget -4 -q --show-progress -O "$tmp_script" \
        "https://raw.githubusercontent.com/taifunss/FirewallFalcon-Manager/main/menu.sh"; then
        chmod +x "$tmp_script"
        local target; target=$(command -v menu 2>/dev/null || echo "/usr/local/bin/menu")
        mv "$tmp_script" "$target"
        chmod +x "$target"
        echo -e "\n${C_GREEN}✅ Skrypt został pomyślnie zaktualizowany do najnowszej wersji!${C_RESET}"
        echo -e "${C_DIM}Ścieżka: $target${C_RESET}"
        echo -e "\n${C_YELLOW}⚠️ Uruchom 'menu' ponownie żeby użyć nowej wersji.${C_RESET}"
        press_enter
        exit 0
    else
        rm -f "$tmp_script"
        echo -e "\n${C_RED}❌ Błąd: Nie udało się pobrać skryptu. Sprawdź połączenie internetowe.${C_RESET}"
        press_enter
    fi
}
...
main_menu() {
    while true; do
        ...
        echo
        echo -e "   ${C_TITLE}═══════════════════[ ${C_BOLD}👤 ZARZĄDZANIE UŻYTKOWNIKAMI ${C_RESET}${C_TITLE}]═══════════════════${C_RESET}"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "1"  "✨ Utwórz użytkownika"      "2"  "🗑️  Usuń użytkownika"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "3"  "🔄 Odnów konto"             "4"  "🔒 Zablokuj użytkownika"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "5"  "🔓 Odblokuj użytkownika"   "6"  "✏️  Edytuj dane użytkownika"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "7"  "📋 Lista użytkowników"      "8"  "📱 Generuj konfigurację klienta"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "9"  "⏱️  Utwórz konto testowe"   "10" "📊 Zużycie pasma"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "11" "👥 Masowe tworzenie kont"   "21" "👤 Konta użytkowników"

        echo
        echo -e "   ${C_TITLE}══════════════[ ${C_BOLD}🌐 VPN I PROTOKOŁY ${C_RESET}${C_TITLE}]═══════════════════${C_RESET}"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "12" "🔌 Menedżer protokołów"     "13" "📈 Monitor ruchu (Lite)"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s\n"                                   "14" "🚫 Blokada torrentów (Anti-P2P)"

        echo
        echo -e "   ${C_TITLE}══════════════[ ${C_BOLD}⚙️ USTAWIENIA SYSTEMU ${C_RESET}${C_TITLE}]════════════════${C_RESET}"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "15" "☁️  Domena CloudFlare"       "16" "🎨 Konfiguracja banera SSH"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "17" "🔄 Auto-restart"             "18" "💾 Kopia zapasowa"
        printf "     ${C_CHOICE}[%2s]${C_RESET} %-30s ${C_CHOICE}[%2s]${C_RESET} %-30s\n" "19" "📥 Przywróć dane"            "20" "🧹 Wyczyść wygasłe konta"
        printf "     ${C_ACCENT}[%2s]${C_RESET} %-30s\n"                                   "22" "🔄 UPDATE SCRIPT"

        echo
        echo -e "   ${C_DANGER}═══════════════════[ ${C_BOLD}🔥 STREFA NIEBEZPIECZNA ${C_RESET}${C_DANGER}]═══════════════════${C_RESET}"
        echo -e "     ${C_DANGER}[99]${C_RESET} Odinstaluj skrypt               ${C_WARN}[ 0]${C_RESET} Wyjście"
        echo
